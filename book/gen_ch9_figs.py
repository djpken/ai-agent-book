...
此文件生成的圖實際是當前【第 10 章】內容（多 Agent 協作），但仍保存為 fig9-*.svg，
若運行會覆蓋當前正確的【第 9 章】（多模態與實時交互）圖，導致內容錯位。

當前正確的第 9 章 SVG（多模態/語音/Computer Use/VLA/Sim2Real）由
2026-03-12 提交 a33c88f 從原 fig8-*.svg 重命名而來；當前正確的第 10 章 SVG
（多 Agent 協作）由本文件原本生成的內容**手工遷移**到 fig10-*.svg。

要重新生成本文件中的圖，應改為生成 fig10-*.svg 並對照 chapter10.md 引用名。
...
Original (now Ch 10) figure list:
  ...
  fig10-3:  Proposer-Reviewer 循環 (Slidev PPT iterative feedback)
...
def fig9_1():
    ...
    s.text(rx + 15 + ctx_w // 2, fs_y + 16, '共享文件系統', size=FS_SMALL, bold=True)
    files = ['glossary.json', 'chapter3_zh.md', 'review_report.md']
    s.mono(rx + 27, fs_y + 38, '  '.join(files), size=11)
    s.text(rx + 15 + ctx_w // 2, fs_y + 55, '+ 工具調用參數傳遞結構化數據', size=FS_TINY, fill='text_light')

    s.text(rx + col_w // 2, fs_y + 82, '✓ 模塊化 · 可擴展 · 並行', size=FS_SMALL, fill='text_light')
    s.text(rx + col_w // 2, fs_y + 104, '✗ 信息同步複雜', size=FS_SMALL, fill='text_light')
...
# ════════════════════════════════════════════════════════════════════
#  fig9-3: Proposer-Reviewer 循環（Slidev PPT 生成）
# ════════════════════════════════════════════════════════════════════
...
def fig9_3():
    ...
    s.text(W // 2, 28, 'Proposer-Reviewer 循環：Slidev PPT 生成', size=FS_TITLE, bold=True)

    # Editor (Proposer)
    ex, ey, ew, eh = 30, 65, 300, 200
    s.rect(ex, ey, ew, eh, fill='light')
    s.text(ex + ew // 2, ey + 22, 'Proposer Agent', size=FS_BODY, bold=True)
    s.text(ex + 12, ey + 48, '輸入: 論文擴展摘要 (2000 字)', size=FS_TINY, anchor='start', fill='text_light')
    editor_lines = [
        '---',
        'theme: academic',
        '---',
    ...
    s.text((ex + ew + cx) / 2, mid_y1 - 12, 'Slidev 代碼', size=FS_SMALL, bold=True)
    ...
    s.text(dual_x, wy + 63, 'Editor 僅累積文本反饋 → 上下文乾淨', size=FS_TINY, anchor='start')
...
def fig9_4():
    ...
    s.text(mx + mw // 2, my + 46, '任務理解 → 分解 → 調度 → 綜合', size=FS_TINY, fill='text_light')
    ...
    agents = [
        ('Sub-Agent A', '數據收集', '搜索技術文檔\n提取關鍵信息', 'light'),
        ('Sub-Agent B', '分析處理', '對比分析數據\n生成統計報告', 'light'),
    ...
    for i, item in enumerate(flow_items):
        s.text(fx_start + i * 118, dy + 42, item, size=FS_TINY, anchor='start',
               fill='text' if '調用' in item or '返回' in item else 'text_light')

    s.text(W // 2, dy + 65, 'Manager 視角：調用 Agent = 調用工具（發請求 → 獲響應）',
           size=FS_SMALL, fill='text_light')
...
def fig9_5():
    ...
    s.text(W // 2, fy + 18, '共享文件系統', size=FS_BODY, bold=True)
    files = [
        ('glossary.json', '術語對照表'),
    ...
    s.text(W // 2, ky + 42,
           'Glossary: 僅看術語 | Translation: 僅看當前章節+術語表 | Manager: 僅維護文件索引',
...
def fig9_6():
    ...
    s.text(W // 2, 28, 'Manager 並行協調：消息總線架構', size=FS_TITLE, bold=True)

    # Orchestration Agent
    ox, oy, ow, oh = 240, 55, 300, 70
    s.rect(ox, oy, ow, oh, fill='medium')
    s.text(ox + ow // 2, oy + 22, 'Orchestration Agent', size=FS_BODY, bold=True)
    s.text(ox + ow // 2, oy + 48, '並行調度 · 實時監控 · 結果彙總', size=FS_TINY, fill='text_light')

    # Message bus
    bus_y = 155
    s.rect(50, bus_y, W - 100, 36, fill='dark', rx=4)
    s.text(W // 2, bus_y + 18, '消息總線（Message Bus）', size=FS_SMALL, fill='white', bold=True)

    s.arrow(ox + ow // 2, oy + oh + 2, ox + ow // 2, bus_y - 2)

    # Parallel agents
    agents = [
        ('Agent 1', '數據採集', '運行中 ◎', 'light'),
        ('Agent 2', '內容分析', '運行中 ◎', 'light'),
    ...
    s.text(W // 2, my + 18, '消息總線通信示例', size=FS_BODY, bold=True)
...
def fig9_7():
    ...
    comp_pipeline = [
        ('屏幕截圖', '瀏覽器當前頁面', 'medium'),
        ('Vision LLM', '理解頁面結構 + 表單字段', 'light'),
        ('動作規劃', '定位字段 → 規劃輸入序列', 'light'),
        ('執行操作', '點擊 / 輸入 / 提交', 'medium'),
    ]
    ...
    s.text(W // 2, ws_y + 18, 'WebSocket 雙向通信 (ws://localhost:8849)', size=FS_SMALL, fill='white', bold=True)
    ...
    s.text(W // 2, my + 18, '實時雙向消息流（邊打電話邊用電腦）', size=FS_BODY, bold=True)

    msgs = [
        ('Phone → Computer', '[FROM_PHONE_AGENT] 用戶說姓名是張三', '→'),
    ...
    # Key point
    s.text(W // 2, my + 140,
           '關鍵：兩個 Agent 獨立 ReAct 循環並行運行，互不阻塞',
           size=FS_SMALL, fill='text_light')
...
def fig9_8():
    ...
    s.text(ox + ow // 2, oy + 44, '動態創建 · 實時監控 · 級聯終止', size=FS_TINY, fill='text_light')
    ...
    for i, (name, url, status, fill) in enumerate(agents):
        ...
        s.text(x + aw // 2, ay + 55, '教師名錄搜索', size=FS_TINY, fill='text_light')
    ...
    s.text(575, ry + 18, '性能對比', size=FS_BODY, bold=True)
    s.text(420, ry + 42, '串行: 10 個網站 × 30s = ~5 分鐘', size=FS_TINY, anchor='start', fill='text_light')
    s.text(420, ry + 60, '並行: 18 秒找到 + 1 秒終止 = 19 秒', size=FS_TINY, anchor='start', bold=True)
    s.text(420, ry + 78, '加速比: ~15×（含級聯終止優化）', size=FS_TINY, anchor='start', fill='text_light')
...
def fig9_9():
    ...
    nodes = [
        ('Agent A', '需求分析', '輸出: 結構化需求文檔\nspec.json', 'medium'),
        ('Agent B', '架構設計', '輸出: 技術設計文檔\ndesign.md', 'light'),
        ('Agent C', '代碼實現', '輸出: 源代碼\nsrc/*.py', 'light'),
    ...
    handoff_fields = [
        ('觸發條件', 'A 完成需求文檔 → is_complete=True'),
...
def fig9_10():
    ...
    s.text(W // 2, 28, 'MetaGPT SOP 流水線：標準化文檔驅動', size=FS_TITLE, bold=True)

    roles = [
        ('Product Manager', 'medium',
         '輸入: 用戶需求描述',
         ['功能列表 + 優先級', '用戶故事 (5 條)', '驗收標準'],
         'docs/PRD.md'),
        ('Architect', 'light',
         '輸入: PRD.md',
         ['技術棧: FastAPI+React', 'API 規範 (OpenAPI)', '數據庫 Schema'],
         'docs/design.md'),
        ('Engineer ×3', 'light',
         '輸入: design.md + 模塊規約',
         ['模塊 A: 用戶服務', '模塊 B: 訂單服務', '模塊 C: 支付服務'],
         'src/*.py'),
        ('QA Engineer', 'medium',
         '輸入: src/ + PRD.md',
         ['單元測試 (pytest)', '集成測試 (API)', 'Bug 報告 → Engineer'],
         'docs/test_report.md'),
    ]
    ...
    s.text(W // 2, fy + 16, '共享項目目錄', size=FS_SMALL, bold=True)
    ...
    insights = [
        ('標準化文檔', '每個角色輸出明確格式 — 下游只需理解格式，不需理解上游思考過程'),
        ('接口解耦', '改進 PM（換更強模型）只要輸出符合 PRD 格式，下游零修改'),
        ('無 Manager', '控制權沿 DAG 自然流動：PM→Arch→Eng→QA，無中心調度開銷'),
        ('異常通道', 'QA 測試失敗 → Bug 報告按模塊路由回 Engineer → 迭代修復'),
    ]
...
def fig9_11():
    ...
    s.text(W / 2, out_y + 18,
           '機器人控制信號：7-DOF 關節角度 / 末端位姿',
           size=FS_SMALL, bold=True, fill='white')
    s.text(W / 2, out_y + 36,
           '差異在"如何告訴機器人下一步該怎麼動"，最終都落到統一的控制接口',
...
def fig9_12():
    ...
    s.text(W // 2, 28, '實驗 9.9：語音狼人殺 — 信息權限控制', size=FS_TITLE, bold=True)

    # Judge (code-driven)
    jx, jy, jw, jh = 260, 55, 260, 75
    s.rect(jx, jy, jw, jh, fill='dark', rx=6)
    s.text(jx + jw // 2, jy + 20, '法官（代碼驅動）', size=FS_BODY, bold=True, fill='white')
    s.text(jx + jw // 2, jy + 42, '遊戲狀態 · 階段控制 · 信息分發', size=FS_TINY, fill='white')
    ...
    s.text(W // 2, iy + 18, '信息權限控制：法官按角色過濾上下文', size=FS_BODY, bold=True)
    ...
    s.text(W // 2, vy + 18, '實時語音交互（ASR + LLM + TTS）', size=FS_BODY, bold=True)
