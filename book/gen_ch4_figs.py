...
def fig4_10():
    """動態發現後的上下文結構：工具 schema 散落在軌跡各處"""
    w, h = 880, 580
    svg = SVG(w, h)
    svg.text(w / 2, 30, "動態發現後的上下文結構：工具 schema 散落在軌跡各處", size=FS_TITLE, bold=True)

    col_x, col_w = 40, 520

    # 靜態前綴組
    svg.rect(20, 50, col_w + 40, 118, fill='white', stroke='border', dash=True)
    svg.text(36, 72, "靜態前綴（字節級不變，KV Cache 持續命中）", size=FS_SMALL, bold=True, anchor='start')
    ...
    svg.text(col_x + col_w / 2, 141, "核心工具定義：web_search, code_interpreter, tool_search", size=FS_TINY, bold=True)

    # 軌跡組
    svg.rect(20, 180, col_w + 40, 386, fill='white', stroke='border', dash=True)
    svg.text(36, 202, "軌跡（只增不改，新內容追加在末尾）", size=FS_SMALL, bold=True, anchor='start')

    blocks = [
        ("User：查詢 NVDA 股價", 'light', False),
        ("Assistant：tool_search_call(股價)", 'light', False),
        ("tool_search_output → 注入 get_stock_quote 完整 schema", '#d8e8d8', True),
        ("Assistant：調用 get_stock_quote → Tool Result", 'light', False),
        ("User：分析 GitHub 倉庫的貢獻者", 'light', False),
        ("Assistant：tool_search_call(GitHub)", 'light', False),
        ("tool_search_output → 注入 list_contributors 等 schema", '#d8e8d8', True),
        ("Assistant：調用 → Tool Result → 回覆", 'light', False),
        ("…… 本輪最新內容", 'light', False),
    ]
    by = 214
    star_ys = []
    ...
    svg.text(600, star_ys[0] - 12, "首次出現：prefill 一次（緩存寫入）", size=FS_TINY, anchor='start', bold=True)
    svg.text(600, star_ys[0] + 10, "此後作為普通曆史命中緩存", size=FS_TINY, anchor='start', fill='text_light')
    svg.text(600, star_ys[1] - 12, "不得移除/重排已加載工具", size=FS_TINY, anchor='start', bold=True)
    svg.text(600, star_ys[1] + 10, "否則緩存從變動點起失效", size=FS_TINY, anchor='start', fill='text_light')
