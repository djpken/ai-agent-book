...
- 诊断质量取决于 LLM；gpt-4o-mini 在本数据集稳定定位 3 个预置问题（步骤 1 诊断稳定）。步骤 2 生成断言时存在少量不确定性：支付重试问题（RT-002）偶尔会被生成为 `final_status_is:failed` 而非上文示例的 `tool_succeeds`——前者与「修复后网关重试成功」的被测系统行为不符，会让修复后重放显示 2/3 而非 3/3。这属于弱模型的断言选择波动（重跑或换更强模型即可复现 3/3），`--smoke` 内置用例则确定性给出 3/3。
